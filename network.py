...
class Network(object):
    ...
    def conv(self,
        ...
        with tf.name_scope(name), tf.variable_scope(name):
            ...
            print(x_tensor)
            c_i = x_tensor.get_shape().as_list()
            print(c_i)
            # Create the weights, with shape [k_h, k_w, c_i, c_o]
            weights = self.make_cpu_variables("weights", [k_h, k_w, c_i[-1]/group, c_o], trainable=trainable)
            ...
            if group == 1:
                outputs = conv2d(x_tensor, weights)
            # If we need to divide this convolutional layer
            else:
                # Split the input and weights
                group_inputs = tf.split(x_tensor, group, 3, name="split_inputs")
